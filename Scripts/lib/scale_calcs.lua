-- scale_calcs.lua
-- v1.2
-- originally written as scale_image.lua by: lzlis
-- moved to this file by: illumini9
-- the body of the scale_image.lua file, moved here
-- so that multiple files can use the process
--
-- changes:
...
local function do_scale(image, scale, out)

  -- added by illumini9; finds the folder of image (empty
  -- string if in base AigisTools folder), finds image
  -- name without path (and extention)
  local out_folder = out
  local imagename
  local lastslash = image:reverse():find('\\')
  if lastslash then
    imagename = image:sub(-lastslash,-5)
  else
    imagename = image:sub(1,-5)
  end
  if out_folder then
    if out_folder:sub(-1) ~= "\\" then
      out_folder = out_folder .. "\\"
    end
  else
    if lastslash then
      out_folder = image:sub(1,-lastslash)
    else
      out_folder = ""
    end
  end
  ...
  gm(false, "convert -size " .. out_width .. "x" .. out_height .. " " ..
    ...
    out_folder .. imagename .. "_scaled.png")
